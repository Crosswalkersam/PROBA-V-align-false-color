...
#This Script assumes all channels are aligned and have the same size.
...
if(aligned == False):
    ...
    ChannelA.save("V1_resized.png") #Noneed to resize V1, its our base image

    ChannelB_width = ChannelB.width + 12 #Stretch image 6pixels left and right
    ChannelB = ChannelB.resize((ChannelB_width, ChannelB.height))
    ChannelB = ImageChops.offset(ChannelB, -6, 104)  #Shift V2 104 pixels down
    print(Fore.GREEN + "Saving Channel 2...")
    ChannelB.save("V2_resized.png")

    ChannelC = ImageOps.mirror(ChannelC)
    ChannelC_width = ChannelC.width + 16 #Stretch image 16pixels left and right
    ChannelC = ChannelC.resize((ChannelC_width, ChannelC.height))
    ChannelC = ImageChops.offset(ChannelC, -8, 158)
...
for x in tqdm(range(ChannelA.width)):
    for y in range(ChannelA.height):
        try:
            A_value = ChannelA.getpixel((x,y)) / 65536 * 256 #Convert 16Bit Greyyscale to 8 Bit RGB
...
Green.crop((0, 158, ChannelB_width, Green.height - 158))
RGB.crop((0, 158, ChannelB_width, RGB.height - 158))
Water.crop((0, 158, ChannelB_width, Water.height - 158))
